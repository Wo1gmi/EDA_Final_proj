"""Загрузка и проверка зафиксированного снимка исходных данных."""
...
def write_checksums() -> None:
    ...
    for f in [cfg.RAW_KEYRATE_FILE, cfg.RAW_USDRUB_FILE, cfg.RAW_BRENT_FILE]:
        ...
        if expected and expected != digest:
            raise RuntimeError(
                f"Хеш {f.name} не совпадает с зафиксированным при первом прогоне "
                f"({expected[:12]}... vs {digest[:12]}...) - текущий снимок отличается от зафиксированного. "
                "Восстановите приложенные данные либо осознанно обновите снимок, ожидаемые хеши и все результаты."
            )
