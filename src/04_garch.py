...
Версия 2 (после ревью, см. `feedback_1.md`):
- GARCH-спецификация выбирается сравнением по AIC/BIC (Normal/t,
  ...
  Исходная спецификация (const-mean, normal) не проходила диагностику
  остатков (Ljung-Box p<0.001) — это была недоработка, а не ограничение.
- Событийное исследование переписано: наивный тест (плацебо равномерно по
  всей выборке) не учитывал, что 19 из 65 решений физически лежат в
  кризисных годах (2014/2015/2022) — самых волатильных периодах на графике
  R3. Новый тест использует плацебо, стратифицированное по году, и
  аномальную |доходность| (за вычетом базового уровня за 30 торговых дней
  до окна). Наивный тест оставлен рядом как явное сравнение "было/стало".
...
СД без изменения ставки" не построена — открытая история всех заседаний
(не только тех, где меняли ставку) на cbr.ru доступна без дополнительного
парсинга только с 2024 года, для 2013-2023 понадобится отдельный источник.
Без неё тест остаётся ослабленным: подтверждаем реакцию на волатильность
в целом вокруг решений, а не специфичность именно к решению как событию.
...
def build_t5(df: pd.DataFrame, spec_kwargs: dict) -> tuple[pd.DataFrame, pd.Series]:
    ...
    for label, sub in periods.items():
        ...
        # это НЕ базовая/безусловная дисперсия, это константа уравнения (P1-3).
        persistence = alpha + beta + gamma / 2
        # Персистентность ~1 (интегрированный GARCH) — известный артефакт на
        # подвыборках с кластером экстремальной волатильности (см. фидбек,
        # P1-3 п.4): безусловная дисперсия в этом случае не идентифицируется
        # устойчиво, честно репортим NaN, а не астрономическое число.
        denom = 1 - persistence
...
def event_study_naive(df: pd.DataFrame, cond_vol: pd.Series, window: int) -> dict:
    """Исходная версия теста (плацебо равномерно по всей выборке, без учёта
    кластеризации решений в кризисных годах). Оставлена для явного сравнения
    "было/стало" — см. feedback_1.md, P0-1."""
    n = len(df)
...
def _abnormal_stat(centers: np.ndarray, abs_ret: np.ndarray, window: int, n: int) -> float:
    ...
    "аномальная" величина, а не сырой уровень (P0-1, пункт 2)."""
    vals = []
    for c in centers:
        lo, hi = max(c - window, 0), min(c + window, n - 1)
...
def event_study_stratified(df: pd.DataFrame, window: int, restrict_years: set | None = None) -> dict:
    ...
    кризисные окна со спокойными днями (P0-1, пункт 1)."""
...
def event_study_before_after(df: pd.DataFrame, window: int) -> dict:
    """Разбивка окна на 'до решения' и 'после решения' + базовый уровень
    за 30 дней до окна (P0-1, пункт 3)."""
...
def main() -> None:
    ...
    print("\n=== T6a: наивный тест (до правок по фидбеку) ===")
    ...
    # --- До/после решения ---
    before_after = event_study_before_after(df, cfg.EVENT_WINDOW_DAYS)
    t6c = pd.DataFrame([before_after])
    ...
    # Сохраняем и старую T6 (наивный тест) для обратной совместимости ссылок в README/плане
